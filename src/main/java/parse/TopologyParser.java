...
public class TopologyParser {
    ...
    public static void parse(String json) throws JSONException {
        ...
        JSONArray fogDevicesNode = root.getJSONArray("fog_devices");
        ...
        List<Datacenter> fogDevices = root.getJSONArray("fog_devices").toList().stream().map(fogDevice -> {
            ...
            List<Host> hosts = fogDeviceObj.getJSONArray("hosts").toList().stream().map(host -> {
                ...
                List<Vm> vms = hostObj.getJSONArray("vms").toList().stream().map(vm -> {
                    ...
                    String vmm = getOr(vmObj, "vmm", "Xen", String.class);
                    String cloudlet_scheduler = getOr(vmObj, "cloudlet_scheduler", "SpaceShared", String.class);

                    return new Vm(vmId, hostId, mips, numOfPes, vmRam, vmBw, size, vmm, new CloudletSchedulerSpaceShared());
                }).collect(Collectors.toList());
    ...
    private static <T> T getOr(JSONObject jsonObject, String key, T defaultValue, Class<T> type) {
