...
public class Parser {
    ...
    public List<Workflow> parse() throws JSONException {
        ...
        return root.getJSONArray(Tags.WORKFLOWS).toList().stream().map(workflowNode -> {
            ...
            List<TaskInfo> tasksInfo = workflowObj.getJSONArray(Tags.TASKS).toList().stream().map(task -> {
                ...
                // Parse input files
                List<misty.parse.dax.File> inputFiles = taskObj.getJSONArray(Tags.INPUT_FILES).toList().stream().map(file -> {
                    ...
                    return new misty.parse.dax.File(file_id, size);
                }).collect(Collectors.toList());

                // Parse output files
                List<misty.parse.dax.File> outputFiles = taskObj.getJSONArray(Tags.OUTPUT_FILES).toList().stream().map(file -> {
                    JSONObject fileObj = new JSONObject((Map<?, ?>) file);
                    String file_id = fileObj.getString(Tags.FILE_ID);
                    long size = fileObj.getLong(Tags.SIZE);
                ...
                long inputFileSize = inputFiles.stream().map(misty.parse.dax.File::getSize).reduce(0L, Long::sum);

                long outputFileSize = outputFiles.stream().map(misty.parse.dax.File::getSize).reduce(0L, Long::sum);
    ...
    private class TaskInfo {
        ...
        public List<misty.parse.dax.File> inputFiles;
        public List<misty.parse.dax.File> outputFiles;
